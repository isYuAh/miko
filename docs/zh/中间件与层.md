...
Miko 完全兼容 Tower 生态的中间件，包括那些可能会修改 Body 类型（如 `CompressionLayer`）或抛出错误（如 `TimeoutLayer`）的中间件。

### 支持的中间件类型

- **Infallible Middleware**: 不会报错的中间件（如 `TraceLayer`）。
- **Fallible Middleware**: 可能会报错的中间件（如 `TimeoutLayer`, `RateLimitLayer`）。
  - Miko 会自动捕获这些错误，将其转换为 `AppError`，最终生成统一的 JSON 错误响应（或符合 HTTP 语义的响应）。
- **Body Modifying Middleware**: 修改响应体的中间件（如 `CompressionLayer`）。
  - Miko 会自动适配不同的 Body 类型。
